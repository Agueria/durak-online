...
import { canDefend, canAttackAppend, areAllAttacksDefended } from './rules.js';
/**
 * Saldırı kartı oyna
 */
export function playAttack(gameState, players, action) {
 ...
 * Savunma kartı oyna
 */
export function playDefense(gameState, players, action) {
    const { playerId, payload } = action;
 ...
 */
export function takeCards(gameState, players, playerId) {
    // Faz kontrolü
    if (gameState.phase !== GAME_PHASES.DEFENDING) {
        return { success: false, error: 'Savunma fazında değil' };
    }
 ...
 */
export function endTurnResolve(gameState, players, deck) {
    // Faz kontrolü
    if (gameState.phase !== GAME_PHASES.ATTACKING) {
        return { success: false, error: 'Saldırı fazında değil' };
    }
