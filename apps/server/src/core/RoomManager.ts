...
import { createGame, gameReducer, drawUpToSix, isGameOver, getNextTurn, type GameAction } from '@durak/game-engine';
import { GAME_PHASES, type GameSettings, type RoomSummary, type ServerState, type PlayerView } from '@durak/shared';
import { MemoryStore, type Room } from './Adapters/MemoryStore.js';
...
export class RoomManager {
   ...
   * Yeni oda oluşturur
   */
  createRoom(name: string, maxPlayers: number, settings?: GameSettings): Room {
    const roomId = uuidv4();
   ...
   */
  joinRoom(roomId: string, session: Session): { success: boolean; error?: string } {
   ...
   */
  leaveRoom(roomId: string, playerId: string): { success: boolean } {
    const room = this.getRoom(roomId);
   ...
   * Oyunu başlatır
   */
  startGame(roomId: string, starterId: string): { success: boolean; error?: string } {
    const room = this.getRoom(roomId);
   ...
   * Oyun aksiyonu işler
   */
  processGameAction(roomId: string, action: GameAction): { success: boolean; error?: string } {
    const room = this.getRoom(roomId);
   ...
   */
  private processDrawPhase(roomId: string): void {
    const room = this.getRoom(roomId);
   ...
   */
  getServerState(roomId: string, playerId: string): ServerState | null {
    const room = this.getRoom(roomId);
    ...
    const players: PlayerView[] = Array.from(room.players.values()).map(p => ({
      id: p.id,
      nickname: p.nickname,
